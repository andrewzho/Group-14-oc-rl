...
we can consider the memory_state to be part of the environment.
...
from .parallel_envs import MultiEnvWrapper
from .global_settings import STATE_SIZE, STATE_STACK, NUM_ACTIONS
from .model import StateClassifier


class MemoryAugmentedEnv(gym.Wrapper):
    """
    MemoryAugmentedEnv is a single environment wrapper that adds
    state features as part of its observations.
    """

    def __init__(self, env, state_features=None):
        super().__init__(env)
        self.state_features = state_features or FeatureExtractor()
        self.prev_memory_state = np.zeros([STATE_STACK, STATE_SIZE], dtype=np.float32)

    def reset(self, **kwargs):
        obs = self.env.reset(**kwargs)
        self.prev_memory_state.fill(0)
        feats = self.state_features.features(np.array([obs])[..., -3:])
        self.prev_memory_state[-1, NUM_ACTIONS + 1:] = feats
        return (self.prev_memory_state.copy(), obs)

    def step(self, action):
        obs, rew, done, info = self.env.step(action)
        self.prev_memory_state[:-1] = self.prev_memory_state[1:]
        self.prev_memory_state[-1] = np.zeros_like(self.prev_memory_state[-1])
        self.prev_memory_state[-1, action] = 1
        self.prev_memory_state[-1, NUM_ACTIONS] = rew
        feats = self.state_features.features(np.array([obs])[..., -3:])
        self.prev_memory_state[-1, NUM_ACTIONS + 1:] = feats
        if 'extra_reward' in info:
            rew += info['extra_reward']
        return (self.prev_memory_state.copy(), obs), rew, done, info


class BatchedMemoryAugmentedEnv(MultiEnvWrapper):
    """
    BatchedMemoryAugmentedEnv wraps a BatchedEnv and adds state
    features to all the observations.

    This is more efficient than using a batch of MemoryAugmentedEnv
    ...
    def __init__(self, env, state_features=None):
        super().__init__(env)
        self.state_features = state_features or FeatureExtractor()
        self.prev_memory_state = np.zeros([env.num_envs, STATE_STACK, STATE_SIZE], dtype=np.float32)
    ...
    def reset(self):
        ...
        self.prev_memory_state.fill(0)
        self.prev_memory_state[:, -1, NUM_ACTIONS + 1:] = feats
        return (self.prev_memory_state.copy(), obses)
    ...
    def step(self, actions):
        ...
        for i, done in enumerate(dones):
            if done:
                self.prev_memory_state[i].fill(0.0)
            else:
                self.prev_memory_state[i, -1].fill(0.0)
                self.prev_memory_state[i, -1, actions[i]] = 1.0
                self.prev_memory_state[i, -1, NUM_ACTIONS] = rews[i]
        self.prev_memory_state[:, -1, NUM_ACTIONS + 1:] = features
        for i, info in enumerate(infos):
            if 'extra_reward' in info:
                rews[i] += info['extra_reward']
        return (self.prev_memory_state.copy(), obses), rews, dones, infos
