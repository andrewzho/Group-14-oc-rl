...
class TrajectoryBatch:
    ...
    TrajectoryBatchs provide a useful representation which can be
    passed to or produced from anywhere that deals with
    trajectories. For example, a TrajectoryBatch can be produced
    from a series of demonstrations, or it can be produced
    by running an agent on a batch of environments for a
    certain number of timesteps. You can pass a TrajectoryBatch to
    an RL algorithm like ProximalPolicyTrainer, or you can feed it to a
    discriminator like the one in GAIL.

    The dones, obses, memory_state, and model_outs are one
    ...
    generated the trajectory_stores. Typically it will have these
    keys:
    ...
    def __init__(self, memory_state, obses, rews, dones, infos, model_outs):
        self.memory_state = np.array(memory_state, dtype=np.float32)
    ...
    def copy(self):
        return TrajectoryBatch(self.memory_state, self.obses, self.rews, self.dones, self.infos, self.model_outs)
